...
<style>
    ...
    /* FORCE SIDEBAR COLORS - MUST BE LAST TO OVERRIDE ADMINKIT */
    .sidebar {
        background: linear-gradient(180deg, #4facfe 0%, #00f2fe 100%) !important;
        box-shadow: 4px 0 20px rgba(79, 172, 254, 0.2) !important;
    }
...
$(document).ready(function() {
    ...
    // Nuclear option: Replace entire breadcrumb content
    function fixBreadcrumb() {
        // Get current page title
        var pageTitle = $('title').text() || 'Analytics';
        if (pageTitle.includes(' - ')) {
            pageTitle = pageTitle.split(' - ')[0];
        }
        
        // Replace entire breadcrumb content
        var newBreadcrumb = `
        ...
        // Replace the breadcrumb content
        $('.breadcrumb').html(newBreadcrumb);
        console.log('Breadcrumb completely replaced');
        
        // Also try to replace any existing breadcrumb
        $('.navbar-breadcrumb .breadcrumb').html(newBreadcrumb);
        $('#custom-breadcrumb').html(newBreadcrumb);
        
        // Force remove any remaining index.html elements
        $('*').contents().filter(function() {
            return this.nodeType === 3 && this.textContent.includes('index.html');
        }).remove();
        
        // Force hide any elements with index.html
        $('*').each(function() {
            if ($(this).text().includes('index.html')) {
                $(this).hide();
            }
        });
    }
    
    // Fix breadcrumb on page load and after DOM changes
    fixBreadcrumb();
    ...
    // Fallback: Run fixBreadcrumb every second for the first 10 seconds
    var breadcrumbFixInterval = setInterval(function() {
        fixBreadcrumb();
    }, 1000);
    
    // Stop the interval after 10 seconds
    setTimeout(function() {
        clearInterval(breadcrumbFixInterval);
    }, 10000);
});
</script>
