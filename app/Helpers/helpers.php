...
function saveCustomerWithDeliverAddress($request){
    try {
        ...
        $user = Customer::where('insuranceNumber', $insurance_no)->first();
        Session::put('Customer', $user);

        $carts = $request->session()->get('cart');

        
         //echo $insurance_no. '--'.$user['healthInsuranceNo'];
        
        //return $user->id;
        /*** If existing user try  to make new order with existing insurance number then it will be update. **/
        if(isset($carts) ){
            $customer = [];
            ...
            {
            $customer_updt->password = Hash::make($get_personalData[0]->password);
            $customer_updt->view_password = $get_personalData[0]->password;
            }
